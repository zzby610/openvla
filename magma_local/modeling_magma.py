...
class MagmaCausalLMOutputWithPast(ModelOutput):
    ...
    # image_num_patches: Optional[List[int]] = None
    action_accuracy: Optional[torch.FloatTensor] = None
...
class MagmaForCausalLM(MagmaPreTrainedModel):
    ...
    # image_features = image_features.to(inputs_embeds.dtype)#新加的
    def _merge_input_ids_with_image_features(
        ...
        with torch.no_grad():
            ...
            # print("[DEBUG] input_ids contains image token:", (input_ids == image_token_index).any())
            # print("[DEBUG] special_image_token_mask type:", type(special_image_token_mask))
            # print("[DEBUG] special_image_token_mask content:", special_image_token_mask)

            # special_image_token_mask: [bsz, seqlen]
            # print(type(special_image_token_mask), isinstance(special_image_token_mask, torch.Tensor))
        ...
        # print("[DEBUG] image_features dtype:", image_features.dtype)
        # print("[DEBUG] inputs_embeds dtype:", inputs_embeds.dtype)
    ...
    def forward(
        ...
        if labels is not None and self.training:
            # action_token_start = 128261
            # action_token_end = 128516
            action_end = 128000
            valid_mask = labels[..., 1:] != -100
            shift_logits = self.language_model.lm_head(hidden_states[:,:-1][valid_mask]).contiguous()
            # shift_logits = shift_logits.view(-1, action_end)
            # logits = shift_logits # dummy logits
            shift_labels = labels[..., 1:][valid_mask].contiguous()
            shift_labels = shift_labels.to(shift_logits.device)

            #调整权重 focus on action
            # weights = torch.ones(self.language_model.config.vocab_size, device=shift_logits.device)
            # weights[action_token_start:action_token_end + 1] = 2.0
            # probs = torch.softmax(shift_logits, dim=-1)
            
            vocab_size = shift_logits.shape[-1]
            weights = torch.ones(vocab_size, device=shift_logits.device)
            weights[vocab_size-256:vocab_size] = 2.0
            probs = torch.softmax(shift_logits, dim=-1)
            

            #Regular term to promote a flat distribution
            ...
            # print("shift_labels range:", shift_labels.min().item(), shift_labels.max().item())


            # if not hasattr(self.config, "tokenizer_vocab_size"):
            #     self.config.tokenizer_vocab_size = 128000  # ？
                # self.config.tokenizer_vocab_size = self.vocab_size
                # print("tokenizer_vocab_size:", self.config.tokenizer_vocab_size)
            ...
            # valid_indices = (shift_labels >= action_token_start) & (shift_labels <= action_token_end)

            # print("config.vocab_size:", self.config.vocab_size)
            # print("self.language_model.config.vocab_size:", self.language_model.config.vocab_size)


            valid_indices = (shift_labels<action_end) & (shift_labels>=action_end-256)
            # valid_indices = (shift_labels<self.config.tokenizer_vocab_size) & (shift_labels>=self.config.tokenizer_vocab_size-256)
            # print("valid_indices.sum():", valid_indices.sum().item())
            if valid_indices.sum() > 0:
                ...
                # print("action_preds:", action_logits.argmax(-1))
            else:
            ...
            if dist.is_available() and dist.is_initialized():
                ...
                action_accuracy_gather = [action_accuracy]  # fallback 用本地值代替
        ...
        return MagmaCausalLMOutputWithPast(
            ...
            image_num_patches=torch.tensor(image_num_patches, device=logits.device).squeeze(-1) if image_num_patches is not None else None,
            action_accuracy=action_accuracy,
            action_logits=action_logits if valid_indices.sum() > 0 else None,
            action_labels=action_labels if valid_indices.sum() > 0 else None,
        ) 
...
class MagmaForConditionalGeneration(MagmaPreTrainedModel):
    ...
    def forward(
            ...
            if pixel_values is not None and input_ids.shape[1] != 1 and pixel_values.size(0) > 0:
                ...
                if self.config.vision_config['img_anyres_strategy'] == "global":
                    ...
                    for idx, (image_size, pixel_values_for_image) in enumerate(zip(image_sizes, pixel_values_for_images)):
                        ...
                        selected_image_feature = image_features[vision_feature_layer][0].permute(1, 2, 0)
                        selected_image_feature = self.multi_modal_projector(selected_image_feature)
                        selected_image_feature = selected_image_feature.to(inputs_embeds.dtype)
                        selected_image_feature = torch.cat((selected_image_feature, self.multi_modal_projector.row_seperator.repeat(selected_image_feature.shape[0],1,1)), dim=1)
                ...
                image_features = torch.cat([elem.flatten(0, 1) for elem in selected_image_features], 0).to(inputs_embeds.dtype)
                image_features = image_features.to(inputs_embeds.dtype)
                feature_lens = torch.tensor(feature_lens, dtype=torch.long, device=image_features.device)

                # print("[DEBUG] image_features final dtype (before merge):", image_features.dtype)
                # print("[DEBUG] inputs_embeds dtype:", inputs_embeds.dtype)
        ...
        l# Keep full logits for later use
        logits = outputs[0]  # shape: [B, T, V]

        # Compute loss separately
        if labels is not None:
            # Compute shifted logits and labels only for loss
            if attention_mask is not None:
                shift_attention_mask = attention_mask[..., 1:]
                valid_mask = shift_attention_mask.to(logits.device) != 0
                shift_logits = logits[..., :-1, :][valid_mask]
                shift_labels = labels[..., 1:][valid_mask]
            else:
